...
public class Odometer extends OdometerData implements Runnable {
  ...
  public static int lastTachoL;	//tacho L last sample
  public static int lastTachoR;	//tacho R last sample
  ...
  public void run() {
    ...
    while (true) {
      ...
      distL = Math.PI * WHEEL_RAD * (nowTachoL - lastTachoL) / 180;	//compute wheel
      distR = Math.PI * WHEEL_RAD * (nowTachoR - lastTachoR) / 180;	//displacement
      lastTachoL = nowTachoL;						//save tacho counts for next iteration
      lastTachoR = nowTachoR;
      deltaD = 0.5*(distL+distR);					//compute the vehicle displacement
      deltaT = (distL-distR) / TRACK; 				//compute the change in heading
